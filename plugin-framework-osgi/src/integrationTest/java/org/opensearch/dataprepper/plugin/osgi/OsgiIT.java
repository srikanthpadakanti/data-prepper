...
class OsgiIT {
    ...
    private DefaultPluginFactory createDefaultPluginFactory() {
        ...
        // OsgiFrameworkRunner injects the application's composite registry rather than using
        // Metrics.globalRegistry. The dependency is optional, so this bean is registered
        // deliberately: it makes this test exercise the injected-registry path even though
        // MetricsConfig is not part of this context.
        // An explicit lambda rather than a constructor reference: CompositeMeterRegistry is
        // overloaded, so the method reference is ambiguous against registerBean's overloads.
        applicationContext.registerBean(CompositeMeterRegistry.class, () -> new CompositeMeterRegistry());
