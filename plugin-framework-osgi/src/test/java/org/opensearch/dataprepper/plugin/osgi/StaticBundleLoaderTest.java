...
class StaticBundleLoaderTest {
    ...
    @Test
    void loadBundles_does_not_count_a_fragment_bundle_as_started() throws Exception {
        final File hostJar = createFragmentHostBundle("fragment-host");
        final File fragmentJar = createFragmentBundle("fragment-attachment", "test.fragment-host");

        final List<Bundle> result = loader.loadBundles(Arrays.asList(hostJar, fragmentJar));

        // Fragments attach to their host at resolve time and cannot be started, so only the
        // host appears in the started list and only the host is counted as loaded.
        assertThat(result.size(), is(1));
        assertThat(result.get(0).getSymbolicName(), is("test.fragment-host"));
        assertThat(result.get(0).getState(), is(Bundle.ACTIVE));

        final Counter loaded = meterRegistry.find(StaticBundleLoader.METRIC_BUNDLES_LOADED).counter();
        assertThat(loaded, is(notNullValue()));
        assertThat(loaded.count(), is(1.0));
    }
    ...
    void loadBundles_with_non_osgi_jar_throws_fail_fast_with_actionable_message() throws Exception {
        ...
        assertThat(ex.getMessage(), containsString("org.opensearch.dataprepper.plugin"));
    ...
    /**
     * A host for a fragment must declare Bundle-ManifestVersion 2 so that it exposes an
     * osgi.wiring.host capability for the fragment to attach to.
     */
    private File createFragmentHostBundle(final String name) throws IOException {
        final File jar = new File(tempDir, name + ".jar");
        final Manifest manifest = new Manifest();
        manifest.getMainAttributes().put(Attributes.Name.MANIFEST_VERSION, "1.0");
        ...
        try (JarOutputStream jos = new JarOutputStream(new FileOutputStream(jar), manifest)) {
            // Empty bundle acting as a fragment host
        }
        return jar;
    }

    private File createFragmentBundle(final String name, final String hostSymbolicName) throws IOException {
        final File jar = new File(tempDir, name + ".jar");
        final Manifest manifest = new Manifest();
        manifest.getMainAttributes().put(Attributes.Name.MANIFEST_VERSION, "1.0");
        manifest.getMainAttributes().putValue("Bundle-ManifestVersion", "2");
        manifest.getMainAttributes().putValue("Bundle-SymbolicName", "test." + name);
        manifest.getMainAttributes().putValue("Bundle-Version", "1.0.0");
        // Fragment-Host makes this a fragment which attaches to the host rather than being started
        manifest.getMainAttributes().putValue("Fragment-Host", hostSymbolicName);
        try (JarOutputStream jos = new JarOutputStream(new FileOutputStream(jar), manifest)) {
            // Empty fragment bundle
        }
        return jar;
    }
