...
class FelixPluginManagerTest {
    ...
    @Test
    void tmpdir_fallback_cache_dir_contains_process_pid() throws Exception {
        felixPluginManager = new FelixPluginManager();
        felixPluginManager.start();
        final BundleContext ctx = felixPluginManager.getBundleContext();
        ...
        // The PID suffix is only used for the java.io.tmpdir fallback, where concurrently
        // running test JVMs share one directory. A real installation uses a fixed name.
        assertThat("Temp-dir fallback cache dir should contain PID for uniqueness",
    ...
    void default_cache_dir_is_under_data_prepper_dir_when_property_set() throws Exception {
        ...
        assertThat("Cache dir should use the fixed name so onFirstInit can reclaim it, "
                        + "rather than a process-unique name that would orphan a directory per restart",
                storageProperty, is(new File(tempDir, "data" + File.separator + "osgi"
                        + File.separator + FelixPluginManager.CACHE_DIR_NAME).getPath()));
    }

    @Test
    void default_cache_dir_is_stable_across_framework_instances() throws Exception {
        System.setProperty(FelixPluginManager.DATA_PREPPER_DIR_PROPERTY, tempDir.getAbsolutePath());

        felixPluginManager = new FelixPluginManager();
        felixPluginManager.start();
        final String firstStorage =
                felixPluginManager.getBundleContext().getProperty(Constants.FRAMEWORK_STORAGE);
        felixPluginManager.close();

        felixPluginManager = new FelixPluginManager();
        felixPluginManager.start();
        final String secondStorage =
                felixPluginManager.getBundleContext().getProperty(Constants.FRAMEWORK_STORAGE);

        assertThat("A restart must reuse the same cache directory so it is not orphaned",
                secondStorage, is(firstStorage));
    }

    @Test
    void startup_purges_process_suffixed_cache_dirs_left_by_earlier_versions() throws Exception {
        System.setProperty(FelixPluginManager.DATA_PREPPER_DIR_PROPERTY, tempDir.getAbsolutePath());
        final File osgiDir = new File(tempDir, "data" + File.separator + "osgi");
        final File staleCache = new File(osgiDir, FelixPluginManager.CACHE_DIR_NAME + "-999999");
        final File staleBundle = new File(staleCache, "bundle1");
        assertThat("Test setup should create the stale cache tree", staleBundle.mkdirs(), is(true));
        assertThat("Test setup should create a file inside the stale cache",
                new File(staleBundle, "bundle.info").createNewFile(), is(true));

        felixPluginManager = new FelixPluginManager();
        felixPluginManager.start();

        assertThat("A cache directory orphaned by an earlier process-suffixed naming scheme "
                        + "must be removed at startup",
                staleCache.exists(), is(false));
    }
